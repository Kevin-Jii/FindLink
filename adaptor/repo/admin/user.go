...
func (a *AdminUser) GetUserByUsername(ctx context.Context, username string) (*model.AdminUser, error) {
	qs := query.Use(a.db).AdminUser
	return qs.WithContext(ctx).Where(qs.Name.Eq(username)).First()
}
